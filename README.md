...
- `reduce`：前后检测仍走 Ollama；rewrite 优先走 Azure GPT-4o，Azure 失败后才回退 OpenRouter `qwen/qwen-plus`
...
[services/checker.py]  主编排层
    ...
    ├── scan 主检测     固定 Ollama
    └── reduce 改写     Azure GPT-4o → OpenRouter qwen/qwen-plus（fallback）
```
...
> 仅 Pro 套餐可用
> 请求体仍接受 `api_source=ollama / azure / openrouter`，但当前 rewrite 提供方固定为 `Azure -> OpenRouter fallback`。
...
- 文本前检和后检固定走 Ollama
- rewrite 固定优先走 Azure `gpt-4o` deployment
- 只有 Azure 失败时，才回退到 OpenRouter `qwen/qwen-plus`
...
# Azure OpenAI（reduce rewrite 第一优先级）
AZURE_API_KEY=
AZURE_ENDPOINT=
...
# OpenRouter（reduce rewrite 兜底 / 远端复核可选）
OPENROUTER_API_KEY=
